...
module.exports = (osm, opts) => {
	...
	let analyzefeaturesFromXml = osm => {
		...
		xmlParser.on('</osm.relation.member>', (node, parent) => {
			with (node) {
				let relation = refElements[`relation/${parent.id}`];
				let member = {
					type: type,
					role: node.role? role : '',
					ref: ref
		...
		xmlParser.on('</osm.relation.bounds>', (node, parent) => {
			with (node)
				refElements[`relation/${parent.id}`].setBounds([parseFloat(minlon), parseFloat(minlat), parseFloat(maxlon), parseFloat(maxlat)]);
		});

		xmlParser.on('</osm.relation.tag>', (node, parent) => {
			refElements[`relation/${parent.id}`].addTag(node.k, node.v);
		});
